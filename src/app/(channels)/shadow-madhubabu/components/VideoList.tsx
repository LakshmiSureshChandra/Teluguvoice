...
export default function VideoList({ allVideos }) {
  ...
  const paginatedVideos = remainingVideos.slice(indexOfFirstVideo, indexOfLastVideo);
    ...
    <>
      ...
      <div className="mb-4">
        ...
        {searchTerm && (
          <p className="text-gray-400 text-sm mt-2 text-center">
            Found {filteredVideos.length} audiobook{filteredVideos.length !== 1 ? 's' : ''}
          </p>
        )}
      </div>
      ...
      <div className="space-y-4">
        ...
        {latestVideo && (
          ...
          >
            <h3 className="text-white text-lg font-semibold mb-2">Latest {activeTab === 'shorts' ? 'Short' : 'Episode'}</h3>
            <div className="bg-white/10 rounded-lg overflow-hidden shadow-lg hover:bg-white/15 transition-all duration-300">
              <div className="relative pt-[45%]">  {/* reduced height ratio */}
                <iframe
              ...
              <div className="p-4">  {/* reduced padding */}
                <h3 className="text-white text-lg font-semibold line-clamp-1">{latestVideo.title}</h3>
                <p className="text-gray-300 mt-2 text-sm line-clamp-2">{latestVideo.description}</p>
                ...
                {paginatedVideos.map((video) => (
                  ...
                  >
                    ...
                    <div className="p-4">
                      ...
                      <p className="text-gray-400 mt-2 text-sm line-clamp-3">{video.description}</p>
        ...
        {Array.from({ length: totalPages }, (_, i) => i + 1)
          ...
          .map((number, index, array) => {
            if (index > 0 && array[index - 1] !== number - 1) {
              return [
                <span key={`ellipsis-${number}`} className="px-4 py-2 text-gray-500">...</span>,
                <button
                  key={number}
